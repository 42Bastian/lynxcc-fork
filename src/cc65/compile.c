    ...
    while (CurTok.Tok != TOK_CEOF) {
        ...
        while (1) {
            ...
            */
            ...
            /* The zeropage specifier (__zeropage__ keyword or the equivalent
            ** attribute) is only meaningful for data objects. It cannot apply
            ** to functions or typedefs - reject it there and drop the wrongly
            ** inherited SC_ZEROPAGE flag so we don't emit a .importzp/.exportzp
            ** for a code symbol.
            */
            if ((Entry->Flags & SC_ZEROPAGE) != 0 &&
                (IsTypeFunc (Entry->Type) ||
                 (Decl.StorageClass & SC_TYPEMASK) == SC_TYPEDEF)) {
                Error ("'zeropage' is not valid here");
            ...
            if (Decl.StorageClass & SC_STORAGE) {
                ...
                if (CurTok.Tok == TOK_ASSIGN) {
                    ...
                    /* A zeropage object lives in the ZEROPAGE segment, which is
                    ** of type 'zp' and carries no initialization data - so it
                    ** cannot have an initializer (mirroring a '.res' cell in an
                    ** assembly .zeropage segment).
                    */
                    if (Entry->Flags & SC_ZEROPAGE) {
                        Error ("'zeropage' variable '%s' cannot have an initializer",
                               Entry->Name);
                    }
                    ...
                    */
                    ...
                    if (IsTypeVoid (Decl.Type)) {
                        ...
                        **
                        ** A zeropage object ignores the current bss-name and is
                        ** always emitted into the built-in ZEROPAGE segment,
                        ** whose 'zp' address size makes the label a true
                        ** zero-page symbol.
                        */
